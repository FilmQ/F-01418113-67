...
class Time  
{
    ...
    public:
        ...
        Time add(Time other) {
            // "My shit is solid gold." - Rhetoric - Disco Elysium;          
                                                                   // integral part                           // overflow part                 
        ...
        std::string toString() {
            ...
            */
            ...
            // no string format specifiers?
            if(hour < 10)
                buffer.append("0");
            buffer += to_string(hour);

            buffer += ":";

            if(minute < 10)
                buffer.append("0");
            buffer += to_string(minute);

            buffer += ":";

            if(second < 10)
                buffer.append("0");
            buffer += to_string(second);
